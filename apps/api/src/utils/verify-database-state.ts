...
async function verifyDatabaseState() {
  ...
  try {
    ...
    const { data: userShowsColumns, error: colError } = await serviceSupabase.rpc('exec_sql', {
    ...
    const { data: rlsPolicies, error: rlsError } = await serviceSupabase.rpc('exec_sql', {
    ...
    const { data: rpcFunctions, error: rpcError } = await serviceSupabase.rpc('exec_sql', {
    ...
    const { data: authUidResult, error: authUidError } = await userClient.rpc('exec_sql', {
    ...
    const { error: userInsertError } = await serviceSupabase.from('users').upsert({
      id: testUserId,
      email: 'test@test.com',
    ...
    if (insertError) {
      ...
      await serviceSupabase.from('user_shows').delete().eq('id', insertResult.id);
    }
  } catch (error) {

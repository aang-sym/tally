...
export class AuthenticationFixer {
  ...
  private async verifyAndApplyMigrations(): Promise<{
    ...
    try {
      ...
      const { error: migrationError } = await serviceSupabase.rpc('exec_sql', {
        sql: migrationSQL,
      });
  ...
  private async testJWTAuthentication(): Promise<{
    ...
    try {
      ...
      const { data: authTest, error: authError } = await userClient.rpc('exec_sql', {
  ...
  private async testWatchlistOperations(): Promise<{
    ...
    try {
      ...
      await serviceSupabase.from('user_shows').delete().eq('id', insertTest.id);
  ...
  private async createFallbackShows(): Promise<{
    ...
    try {
      ...
      for (const show of fallbackShows) {
        const { error: upsertError } = await serviceSupabase.from('shows').upsert(show);

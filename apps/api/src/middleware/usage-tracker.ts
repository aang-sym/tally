...
class UsageTracker {
  ...
  getUsageStats(service: 'tmdb' | 'streaming-availability') {
    ...
    const todaySuccessful = serviceData.filter(record => 
      record.timestamp >= today && record.success
    ).length;
    
    const monthSuccessful = serviceData.filter(record => 
      record.timestamp >= thisMonth && record.success
    ).length;
    
    // Calculate average response time for today
    const todayWithResponseTime = serviceData.filter(record => 
      record.timestamp >= today && record.responseTime !== undefined
    );
    
    const avgResponseTime = todayWithResponseTime.length > 0
      ? todayWithResponseTime.reduce((sum, record) => sum + (record.responseTime || 0), 0) / todayWithResponseTime.length
      : 0;

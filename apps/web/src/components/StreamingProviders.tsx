...
const StreamingProviders: React.FC<StreamingProvidersProps> = ({ showId, country, showTitle }) => {
    ...
    const fetchProviders = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/tmdb/show/${showId}/providers?country=${encodeURIComponent(country)}`
        );
  ...
  if (!showId) {
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Streaming Providers</h2>
        <p className="text-gray-500 text-center py-8">Select a show to see where it's available</p>
      </div>
    );
  }
      ...
      {providers.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Try changing the country or checking back later
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          {Object.entries(groupedProviders).map(([type, typeProviders]) => (
            <div key={type}>
              ...
              <div
                className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4 rounded-lg border-2 border-dashed ${getTypeColor(type)}`}
              >
                {typeProviders.map((provider) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-gray-900 text-sm truncate">{provider.name}</p>
          ...
          <div className="bg-gray-50 rounded-lg p-4 mt-6">
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm text-gray-600">
                ...
                <p>
                  Provider information is sourced from TMDB and may not reflect real-time
                  availability. Prices and availability can vary by region and may change
                  frequently.
                </p>
              </div>
            </div>

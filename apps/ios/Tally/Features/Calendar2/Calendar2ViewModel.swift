...
@MainActor
final class Calendar2ViewModel: ObservableObject {
    @Published var country: String = CountryManager.get()
    @Published var monthAnchor: Date = Calendar2ViewModel.firstOfMonth(Date())
    ...
    func changeMonth(by delta: Int) {
        if let newDate = Calendar.current.date(byAdding: .month, value: delta, to: monthAnchor) {
            monthAnchor = Calendar2ViewModel.firstOfMonth(newDate)
        }
    }
